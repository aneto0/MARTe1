 ...
 *
 * Abstract:
 *  The real-time main program for targeting Tornado/VxWorks.
 *  
    ...
    while(1) {
        ...
        for (i = FIRST_TID + 1; i < NUMST; i++) {
            if (sampleHit[i]) {
                 ...
                 * finish in its allotted time period.
                 *
...
/* Function: GetNextStr ========================================================
 * Abstract:
 *  Assuming an input string that consists of space separators return a pointer
...
PARSE_EXIT:
    ...
    for (i = FIRST_TID + 1; i < NUMST; i++) {
        ...
        VxWorksTIDs[i] = taskSpawn(taskName,
            priority + i, VX_FP_TASK, STACK_SIZE, (FUNCPTR)tSubRate, (int_T) rtM,
            (int_T) rtTaskSemaphoreList[i], i,
            0, 0, 0, 0, 0, 0, 0);
    }
    
    VxWorksTIDs[0] = taskSpawn("tBaseRate",
        priority, VX_FP_TASK, STACK_SIZE, (FUNCPTR)tBaseRate, (int_T) rtM, 
    ...
    VxWorksTIDs[0] = taskSpawn("tSingleRate",
        priority, VX_FP_TASK, STACK_SIZE, (FUNCPTR)tSingleRate, (int_T) rtM,
