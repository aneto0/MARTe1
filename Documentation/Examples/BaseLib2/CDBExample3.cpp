...
OBJECTLOADREGISTER(SimpleClass, "$Id: CDBExample3.cpp,v 1.1 2011/08/09 13:46:19 aneto Exp $")
...
int main(int argc, char *argv[]){
    ...
    //We can now look for the object in the GlobalObjectDataBase
    GCRTemplate<SimpleClass> ref = GetGlobalObjectDataBase()->Find("MySimpleClass1");
    if(ref.IsValid()){
        CStaticAssertErrorCondition(Information, "Found MySimpleClass1 as expected");
        CStaticAssertErrorCondition(Information, "The value of MyInt is: %d", ref->GetMyIntValue());
        
    }
    else{
        CStaticAssertErrorCondition(FatalError, "Could not find MySimpleClass1");
    }
