...
#ifndef STREAMINGDRIVER_H_
#define STREAMINGDRIVER_H_
...
OBJECT_DLL(StreamingDriver)

class StreamingDriver: public GenericAcqModule{

OBJECT_DLL_STUFF(StreamingDriver)
...
public:
    ...
    /** Destructor */
    virtual ~StreamingDriver();

    friend void StreamingDriverConsumerThread(void *args);
...
public:
    ...
    virtual bool SetInputBoardInUse(bool on = True){
        ...
        if(inputBoardInUse && on) {
            AssertErrorCondition(InitialisationError, "StreamingDriver::SetInputBoardInUse: Board %s is already in use", Name());
            return False;
        }
    ...
    virtual bool SetOutputBoardInUse(bool on = True){
        ...
        if(outputBoardInUse && on){
            AssertErrorCondition(InitialisationError, "StreamingDriver::SetOutputBoardInUse: Board %s is already in use", Name());
            return False;
        }
