 ...
 *
 * $Id: BinaryFileInputGAM.cpp 462 2016-04-22 12:38:30Z aneto $
 *
**/
...
const int32 BinaryFileInputGAM::MAX32BitsValue = 2.147e9;

BinaryFileInputGAM::~BinaryFileInputGAM(){
...
bool BinaryFileInputGAM::ReadConfigurationFromCDB(CDBExtended &cdb){
    int32  temp                      = 0;    
    ...
    for(int entry = 0; entry < nOfEntries; entry++){
        ...
        if(cdb->Exists("Cal0")){
            ...
            if(!cdb.ReadFloatArray(cal0Pointer,size,maxDim,"Cal0")){
                AssertErrorCondition(InitialisationError,"BinaryFileInputGAM::Initialise: GAM %s: Failed Reading Cal0 Array for signal %s",Name(),signalDescriptor->SignalName());
                return False;
            }
      
            if(!cdb.ReadFloatArray(oldCal0Pointer,size,maxDim,"OldCal0")){
                if(!cdb.ReadFloatArray(oldCal0Pointer,size,maxDim,"Cal0")){
                    AssertErrorCondition(InitialisationError,"BinaryFileInputGAM::Initialise: GAM %s: Failed Reading OldCal0 Array for signal %s",Name(),signalDescriptor->SignalName());
                    return False;
                }
            }
            ...
            if(!cdb.ReadFloatArray(cal1Pointer,size,maxDim,"Cal1")){
                AssertErrorCondition(InitialisationError,"BinaryFileInputGAM::Initialise: GAM %s: Failed Reading Cal1 Array for signal %s",Name(),signalDescriptor->SignalName());
                return False;
            }
...
bool BinaryFileInputGAM::Initialise(ConfigurationDataBase& cdbData){
    ...
    if(!cdb.ReadFString(boardName,"BoardName")){
        AssertErrorCondition(InitialisationError, "BinaryFileInputGAM::Initialise: %s does not specify a BoardName", Name());
        return False;
    }

    GCReference module = GetGlobalObjectDataBase()->Find(boardName.Buffer(),GCFT_Recurse);
    if(!module.IsValid()){
        AssertErrorCondition(InitialisationError, "BinaryFileInputGAM::Initialise: %s: Could not find module %s in Object Containers", Name(),boardName.Buffer());
        return False;
    }

    inputModule = module;
    if(!inputModule.IsValid()){
        AssertErrorCondition(InitialisationError, "BinaryFileInputGAM::Initialise: %s: %s is not of GenericAcqModule Type", Name(),boardName.Buffer());
        return False;
    }
    
    if(!InitialiseTimeInformation(cdb)){
        AssertErrorCondition(InitialisationError, "BinaryFileInputGAM::Initialise: %s: Failed reading Time Informations ", Name());        
        return False;
    }
    ...
    if(!AddOutputInterface(output,"OutputInterface")){
        AssertErrorCondition(InitialisationError,"BinaryFileInputGAM::Initialise: %s failed to add output interface OutputInterface",Name());
        return False;
    }

    // Read Signal Names //
    if(!cdb->Move("Signals")){
        AssertErrorCondition(InitialisationError,"BinaryFileInputGAM::Initialise: %s did not specify Signals entry",Name());
        return False;
    }

    if(!output->ObjectLoadSetup(cdb,NULL)){
        AssertErrorCondition(InitialisationError,"BinaryFileInputGAM::Initialise: %s: ObjectLoadSetup Failed DDBInterface %s ",Name(),output->InterfaceName());
        return False;
    }
    ...
    if(numberOfInputs != inputModule->NumberOfInputs()){
        AssertErrorCondition(InitialisationError,"BinaryFileInputGAM::Initialise: GAM %s specifies %d signal while the module %s only accepts %d ",Name(),numberOfInputs,boardName.Buffer(),inputModule->NumberOfInputs());
        return False;
    }
    ...
    if(cal0 == NULL){
        AssertErrorCondition(InitialisationError,"BinaryFileInputGAM::Initialise: GAM %s : Failed to allocate space for Calibration vector (cal0) of size %d ",Name(),numberOfInputs*sizeof(float));
        return False;
    }

    oldCal0 = (float *)malloc(numberOfInputs*sizeof(float));
    if(oldCal0 == NULL){
        AssertErrorCondition(InitialisationError,"BinaryFileInputGAM::Initialise: GAM %s : Failed to allocate space for Calibration vector (oldCal0) of size %d ",Name(),numberOfInputs*sizeof(float));
        return False;
    }

    cal1 = (float *)malloc(numberOfInputs*sizeof(float));
    if(cal1 == NULL){
        AssertErrorCondition(InitialisationError,"BinaryFileInputGAM::Initialise: GAM %s : Failed to allocate space for Calibration vector (cal1) of size %d ",Name(),numberOfInputs*sizeof(float));
        return False;
    }

    needsCalibration = (bool *)malloc(numberOfInputs*sizeof(bool));
    if(needsCalibration == NULL){
        AssertErrorCondition(InitialisationError,"BinaryFileInputGAM::Initialise: GAM %s : Failed to allocate space for Calibration vector (needsCalibration) of size %d ",Name(),numberOfInputs*sizeof(bool));
        return False;
    }

    noOffsetAdjustment = (int32 *)malloc(numberOfInputs*sizeof(int32));
    if(noOffsetAdjustment == NULL){
        AssertErrorCondition(InitialisationError,"BinaryFileInputGAM::Initialise: GAM %s : Failed to allocate space for Calibration vector (noOffsetAdjustment) of size %d ",Name(),numberOfInputs*sizeof(int32));
        return False;
    }
    ...
    if(offsetCompensation == NULL){
        AssertErrorCondition(InitialisationError,"BinaryFileInputGAM::Initialise: GAM %s : Failed to allocate space for automatic offset compensation (offsetCompensation) of size %d ",Name(),numberOfInputs*sizeof(float));
        return False;
    }
        
    physicalOffsetValue = (float *)malloc(numberOfInputs*sizeof(float));
    if(physicalOffsetValue == NULL){
        AssertErrorCondition(InitialisationError,"BinaryFileInputGAM::Initialise: GAM %s : Failed to allocate space for automatic offset compensation (physicalOffsetValue) of size %d ",Name(),numberOfInputs*sizeof(float));
        return False;
    }
    ...
    if(automaticOffsetCompensation){
        if(!cdb.ReadFloat(percentageCorrection,"MaximumPercentageCorrection")){
            AssertErrorCondition(InitialisationError,"BinaryFileInputGAM::Initialise: GAM %s :MaximumPercentageCorrection has not been specified ",Name());
            return False;
        }
        if(!cdb.ReadInt32(offsetCompensationNumberOfCycles ,"OffsetCompensationNumberOfCycles")){
            AssertErrorCondition(InitialisationError,"BinaryFileInputGAM::Initialise: GAM %s :OffsetCompensationNumberOfCycles has not been specified ",Name());
            return False;
        }
    }
    ...
    if(!inputModule->SetInputBoardInUse()){
        AssertErrorCondition(InitialisationError,"BinaryFileInputGAM::Initialise: %s: Module %s is already being used as input module by another GAM", Name(),inputModule->Name());
        return False;
    }
    ...
    AssertErrorCondition(Warning,"BinaryFileInputGAM::Initialise: %s: Module %s Has been Successfully Loaded", Name(),inputModule->Name());

    return True;
}


bool BinaryFileInputGAM::InitialiseTimeInformation(ConfigurationDataBase& cdbData){
    ...
    if(!AddIOInterface(usecTime,"UsecTimeInterface",DDB_ReadMode)){
        AssertErrorCondition(InitialisationError,"BinaryFileInputGAM::InitialiseTimeInformation: %s failed to add input interface IOInterface",Name());
        return False;
    }

    FString timeBase;
    if(!cdb.ReadFString(timeBase,"UsecTimeSignalName")){
        AssertErrorCondition(InitialisationError, "BinaryFileInputGAM::InitialiseTimeInformation: %s does not specify a UsecTimeSignalName", Name());
        return False;
    }

    FString timeBaseType;
    if(!cdb.ReadFString(timeBaseType,"TimeSignalType","int32")){
        AssertErrorCondition(Warning, "BinaryFileInputGAM::InitialiseTimeInformation: %s does not specify a TimeSignalType. Assuming int32", Name());
    }

    if(!usecTime->AddSignal(timeBase.Buffer(), timeBaseType.Buffer())){
        AssertErrorCondition(InitialisationError,"BinaryFileInputGAM::InitialiseTimeInformation: %s failed to add input Signal %s to interface InputInterface",Name(),timeBase.Buffer());
        return False;
    }
...
int32 BinaryFileInputGAM::ReadTime(){
    usecTime->Read();
...
bool BinaryFileInputGAM::Execute(GAM_FunctionNumbers functionNumber){
    ...
    switch(functionNumber){
        case GAMStartUp:{
            if(inputModule->GetData(time,output->Buffer()) == -1){
                AssertErrorCondition(FatalError,"BinaryFileInputGAM::Execute:: Module %s GetData Failed for driver %s",Name(), inputModule->Name());
                return False;
            }
        ...
        case GAMPrepulse:{
            ...
            if(inputModule->GetData(time, output->Buffer()) == -1){
                AssertErrorCondition(FatalError,"TimeBinaryFileInputGAM::Execute:: Module %s GetData Failed for driver %s",Name(), inputModule->Name());
                return False;
            }
            for(int sig = 0; sig < numberOfInputs; sig++) 
                if(needsCalibration[sig]) {
                ...
                for(int sig = 0; sig < numberOfInputs; sig++){ 
                    if((needsCalibration[sig]) && (startUpCycleNumber != 0)){ 
                        ...
                        if(offsetPercent     > percentageCorrection){
                            AssertErrorCondition(Warning, "BinaryFileInputGAM::Execute:: Module %s: Offset NOT corrected %f >allowed %f for driver %s, signal %d, oldcal0 = %e, newcal0=%e",Name(), offsetPercent , percentageCorrection, inputModule->Name(), sig, oldCal0[sig], cal0[sig]);
...
bool BinaryFileInputGAM::ObjectSaveSetup(ConfigurationDataBase &info, StreamInterface *err){
...
bool BinaryFileInputGAM::ProcessMessage(GCRTemplate<MessageEnvelope> envelope){
...
bool BinaryFileInputGAM::ProcessHttpMessage(HttpStream &hStream){
...
OBJECTLOADREGISTER(BinaryFileInputGAM,"$Id: BinaryFileInputGAM.cpp 462 2016-04-22 12:38:30Z aneto $")
