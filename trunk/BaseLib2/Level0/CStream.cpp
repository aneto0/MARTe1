...
bool CRead(CStream *cs,void *buffer,uint32 &size){
    ...
    while (toDo >0){
        if (cs->sizeLeft==0){
            cs->NewBuffer(cs);
            size -= toDo;
            if (cs->sizeLeft==0) return False;
        }
        uint32 sizeCopy = cs->sizeLeft;
        if (sizeCopy > toDo)sizeCopy = toDo;
        char *bE = pb+sizeCopy;
        while(pb<bE) *pb++ = *(cs->bufferPtr)++;
...
char *CDestructiveGetCStringToken(char *&input,const char *terminator,char *saveTerminator,const char *skip){
...
bool CGetToken(CStream *cs,char *buffer,const char *terminator,uint32 maxSize,char *saveTerminator,const char *skip){
...
bool CGetStringToken(CStream *csIn,CStream *csOut,const char *terminator, int32 totalSize){
...
bool CGetCSToken(CStream *csIn,CStream *csOut,const char *terminator,char *saveTerminator,const char *skip){
