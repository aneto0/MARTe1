...
bool PlottingGAM::Initialise(ConfigurationDataBase &cdbData) {
    ...
    if ((gcrtpw = new GCRTemplate< PlotWindow > [numberOfPlotWindows])
            == NULL) {
        AssertErrorCondition(
                InitialisationError,
                "PlottingGAM::Initialise: %s unable to allocate memory for reference storage",
                Name());
        return False;
    }
    ...
    for (int i = 0; i < numberOfPlotWindows; i++) {
        ...
        if (gcr.IsValid()) {
            ...
            AssertErrorCondition(
                    InitialisationError,
                    "PlottingGAM::Initialise: %s only PlotWindow objects allowed",
                    Name());
            return False;
        }
    }
...
bool PlottingGAM::ProcessHttpMessage(HttpStream &hStream) {
    ...
    hStream.Printf(
            "<script src=\"/RGRAPH_LIB_DIR/RGraph.common.core.js\"></script>\n");
    hStream.Printf(
            "<script src=\"/RGRAPH_LIB_DIR/RGraph.common.zoom.js\"></script>\n");
    hStream.Printf(
            "<script src=\"/RGRAPH_LIB_DIR/RGraph.line.js\"></script>\n");
    hStream.Printf("<script src=\"/RGRAPH_LIB_DIR/RGraph.bar.js\"></script>\n");

    /* Include the js file to plot in line mode */

        hStream.Printf(
                "<script src=\"/PLOTTING_GAM_DIR/PlottingGAM.js\"></script>\n");

    /* Include the js file to plot in histogram mode */
        hStream.Printf(
                "<script src=\"/PLOTTING_GAM_DIR/HistogramBar.js\"></script>\n");
    ...
    for (i = 0; i < numberOfPlotWindows; i++) {
        hStream.Printf(
                "<canvas id='canvas%dtitle' width='%d' height='50' style='position:absolute; left:0px; top:%dpx'></canvas>\n",
                i, gcrtpw[i]->plotPixelWidth, heightCumulative);
        heightCumulative += 50;
        hStream.Printf(
                "<canvas id='canvas%d' width='%d' height='%d' style='position:absolute; left:0px; top:%dpx'></canvas>\n",
                i, gcrtpw[i]->plotPixelWidth, gcrtpw[i]->plotPixelHeight,
                heightCumulative);
        heightCumulative += gcrtpw[i]->plotPixelHeight;
    }
    hStream.Printf(
            "<canvas id='canvasAck' width='400' height='400' style='position:absolute; left:0px; top:%dpx'></canvas>\n",
            heightCumulative + 50);
    hStream.Printf("<script>\n");
...
OBJECTLOADREGISTER(PlottingGAM,
                   "$Id$")
