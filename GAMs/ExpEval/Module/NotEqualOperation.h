...
#ifndef __NOT_EQUAL_OPERATION__
#define __NOT_EQUAL_OPERATION__

#include "BinaryOperation.h"

class NotEqualOperation : public BinaryOperation {

public:

    NotEqualOperation() {
	defaultType = BTDUint32;
	...
	if(!leftData.GetData(BTDFloat , &lv)) {
	    printf("Plus: ExecuteLocal(): unable to get left value\n");
	}
	if(!rightData.GetData(BTDFloat, &rv)) {
