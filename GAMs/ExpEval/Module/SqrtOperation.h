...
#ifndef __SQRT_OPERATION__
#define __SQRT_OPERATION__
...
public:
	...
	if(!data.GetData(defaultType, &dv)) {
	    printf("Sqrt: ExecuteLocal(): unable to get data value\n");
	}

	v = sqrt(dv);
	if(!result.UpdateData(&v)) {
	    printf("Sqrt: ExecuteLocal(): unable to update BasicTypeData value\n");
	}
