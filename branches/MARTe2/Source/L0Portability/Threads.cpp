...
void ThreadsSetInitialisationInterfaceConstructor(ThreadInformationConstructorType threadInfoc){
    if (threadInfoc != NULL){
        ::threadInitialisationInterfaceConstructor = threadInfoc;
    } else {
        ::threadInitialisationInterfaceConstructor = DefaultThreadInformationConstructor;
    }
}

/** Assign default initialisation. */
ThreadInformationConstructorType threadInitialisationInterfaceConstructor = DefaultThreadInformationConstructor;

/** This function allows to call a subroutine within an exception handler protection */
bool ThreadProtectedExecute(ThreadFunctionType userFunction,void *userData, ExceptionHandler *ehi){
...
TID ThreadsBeginThread(ThreadFunctionType function,
    ...
    TID threadId = ThreadsOSBeginThread((StandardThreadFunction)SystemThreadFunction, threadInfo, stacksize, runOnCPUs);
...
void ThreadsSetPriorityLevel(TID threadId, uint32 priorityLevel){
    ...
    if(threadInfo != NULL){
        ...
        ThreadsOSSetPriorityLevel(threadId, threadInfo->priorityClass, priorityLevel);
    }
    ThreadsDatabase::UnLock();
...
void ThreadsSetPriorityClass(TID threadId, uint32 priorityClass){
    ...
    if(threadInfo != NULL){
        ...
        ThreadsOSSetPriorityLevel(threadId, priorityClass, threadInfo->priorityLevel);
    }
    ThreadsDatabase::UnLock();
}
