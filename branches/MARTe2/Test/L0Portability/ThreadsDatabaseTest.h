...
public:
    ...
    /**
     * @see Test the efficiency of the Lock function of the database and compare the thread information returned by database with the thread's attributes.
    ...
    /**
     * @see Test the GetId() function of the database. A thread receives the tid of the other threads in database and compare it with the saved tids in the array. Then, it kills the other threads and
    ...
    /**
     * @see Launchs nOfThreads threads and for each thread the main process control before that its tid is in the database, then remove the entry from the database and check that the informations 
    ...
    /**
     * @see Test the lock of the database with a provided timeout. A thread lock the database and goes in an infinite loop, then another thread locks the database and it must wait until the timeout
